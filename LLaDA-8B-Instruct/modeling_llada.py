...
class LLaDAOutput(NamedTuple):
    ...
    predicted_mask: torch.FloatTensor
    """
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        predicted_mask = self.transformer.mask_head(x)
        
        return LLaDAOutput(logits=logits, predicted_mask=predicted_mask, attn_key_values=attn_key_values, hidden_states=tuple(all_hidden_states) if output_hidden_states else None)  # type: ignore[arg-type]
...
@dataclass
class CausalLMOutputWithPastMask(CausalLMOutputWithPast):
    predicted_mask: torch.FloatTensor = None
...
def generate(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            elif remasking == 'predicted':
                x0_p = torch.sigmoid(predicted_mask.squeeze(-1))
            else:
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
    ...
    ) -> Union[Tuple, CausalLMOutputWithPastMask]:
        ...
        predicted_mask = outputs.predicted_mask
        ...
        return CausalLMOutputWithPastMask(
            logits=logits,
